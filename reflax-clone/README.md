...
`/register` py form fill → data Supabase mein `pending` status se save →
aapko Gmail py email milta hai → aap `/admin` py login kar ke Approve/Reject
karte hain → Approve karte hi freelancer ko automatic email jati hai "aapki
profile approved ho gayi" → profile turant `/hire-freelancers/[category]`
py show hone lagti hai.
